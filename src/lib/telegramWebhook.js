// Telegram Webhook Handler for Two-Way Communication - JavaScript version

// Helper function to send responses
const sendTelegramResponse = async (chatId, text, botToken) => {
  try {
...
const handleTelegramWebhook = async (update) => {
  try {
    if (!update.message) {
      return { ok: true };
    }
    ...
    if (trimmedText.startsWith('/')) {
      ...
      if (commandResult.shouldNotify) {
        ...
        if (botToken) {
          await sendTelegramResponse(chat.id, commandResult.response, botToken);
        }
      }
...
const handleTelegramCommand = (text, userId) => {
